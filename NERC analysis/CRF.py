...
crf.fit(X_train_csv, y_train_csv)

labels_csv = list(crf.classes_)
labels_csv.remove('O')

y_pred_csv = crf.predict(X_test_csv)
metrics.flat_f1_score(y_test_csv, y_pred_csv, 
                      average='weighted', labels=labels_csv)

sorted_labels_csv = sorted(
    labels_csv, 
    key=lambda name: (name[1:], name[0])
)
print(metrics.flat_classification_report(
    y_test_csv, y_pred_csv, labels=sorted_labels_csv, digits=3
))
